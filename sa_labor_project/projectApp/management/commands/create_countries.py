    ...
    def handle(self, *args, **kwargs):
        ...
        for country_path in os.listdir(settings.GEOJSON_PATH):
            ...
            for file in os.listdir(settings.DAILY_AVERAGE_PATH):
                ...
                #self.__create_array(country_folder_path, date_str, tropomi_CO)
                self.__create_geo_json(country_folder_path, date_str, done_list, tropomi_CO)

    def __create_array(self, country_folder_path, date_str, tropomi_CO):
        ...
        with open(os.path.join(country_folder_path, f'{date_str}.geo.json'), 'w') as f:
            f.write(data)

    def __create_geo_json(self, country_folder_path, date_str, done_list, tropomi_CO):
